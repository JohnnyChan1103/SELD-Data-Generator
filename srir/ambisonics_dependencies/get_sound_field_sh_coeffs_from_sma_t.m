...
s_breve = zeros(size(array_signals, 3), (N+1)^2);

for n = 0 : N 
    for m = -n : n        
        
        % evaluate the transformation integral discretely
        s_breve(:, n^2+n+m+1) = sum(sum(array_signals .* repmat(grid_weights .* sphharm(n, m, beta, alpha, 'real'), [1 size(array_signals, 2) 1025]), 2) )* 4*pi;
